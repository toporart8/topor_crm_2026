...
const dateLabel=s=>{const d=parseDate(s);if(!d)return 'Без срока';const day=String(d.getDate()).padStart(2,'0'),m=String(d.getMonth()+1).padStart(2,'0'),y=d.getFullYear();return `${day}.${m}.${y}`};
...
function overview(){const attention=orders.filter(o=>overdue(o)||active(o)&&o.due===today).sort((a,b)=>a.due.localeCompare(b.due));return pageHead('Обзор мастерской','Сроки, задачи и деньги — всё на одном экране.')+stats()+`<div class="dashboard-columns"><section class="panel"><div class="panel-title"><h2>На что обратить внимание</h2></div>${attention.length?attention.map(o=>{const dueInfo=getDueStatus(o);return `<div class="agenda-item"><span class="monogram">${esc(o.client[0])}</span><div><button class="order-name" data-open="${o.id}">${esc(o.client)} · ${esc(o.id)}</button><div class="meta">${esc(o.theme)}</div></div><div style="text-align:right"><div class="due ${dueInfo.isOverdue?'late bold-red':dueInfo.isToday?'warn':''}">${dateLabel(o.due)}</div>${dueInfo.text?`<div class="meta due-meta ${dueInfo.isOverdue?'overdue-alert':dueInfo.isToday?'today-alert':''}">${dueInfo.text}</div>`:''}</div></div>`}).join(''):'<div class="empty">Срочных дел по срокам нет</div>'}</section><section class="panel"><div class="panel-title"><h2>Этапы работы</h2><span class="meta">Активные заказы</span></div><div style="padding:12px 0">${statuses.filter(s=>!['Закрыт','Отказ'].includes(s)).map(s=>{const n=orders.filter(o=>o.status===s).length;return `<div class="stage-row"><span>${s}</span><div class="stage-track"><span style="width:${n/Math.max(orders.filter(active).length,1)*100}%"></span></div><span>${n}</span></div>`}).join('')}</div></section></div>`}
...
document.addEventListener('click',e=>{
 if(e.target.closest('.inline-status, .phone-link, .drag-handle, input, select, textarea'))return;
 const tr=e.target.closest('tr[data-order-id]');
 if(tr&&tr.dataset.orderId){openOrder(tr.dataset.orderId);return;}
 const b=e.target.closest('button');
 if(!b)return;
 if(b.dataset.close){(b.dataset.close==='order'?orderDialog:smallDialog).close();return}
 ...
 if(action==='open-folder'){const url=safeURL(draft().folder);if(!url){toast('Сначала добавьте HTTPS-ссылку на папку');return}window.open(url,'_blank','noopener,noreferrer')}
 if(action==='archive-order'){const o=orders.find(x=>x.id===editing);saveOrder(active(o)?'Закрыт':'Новый')}
});
